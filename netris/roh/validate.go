...
func validateType(val interface{}, key string) (warns []string, errs []error) {
	v := val.(string)
	if !(v == "physical" || v == "hypervisor") {
		errs = append(errs, fmt.Errorf("Invalid type. Available values are (server, hypervisor)"))
	}
	return warns, errs
}

func validateRProfile(val interface{}, key string) (warns []string, errs []error) {
	v := val.(string)
	if !(v == "inherit" || v == "default" || v == "default_agg" || v == "full_table") {
		errs = append(errs, fmt.Errorf("Invalid type. Available values are (inherit, default, default_agg, full_table)"))
	}
	return warns, errs
}

func validatePrefixRule(val interface{}, key string) (warns []string, errs []error) {
	v := val.(string)
